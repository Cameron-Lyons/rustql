...
fn test_concat_two_columns() {
    let _g = setup();
    let result = process_query("SELECT first_name || last_name FROM people WHERE id = 1").unwrap();
    assert!(
...
fn test_concat_with_literal() {
    let _g = setup();
    let result =
        process_query("SELECT first_name || ' ' || last_name FROM people WHERE id = 1").unwrap();
    assert!(
...
fn test_concat_with_number() {
    let _g = setup();
    let result = process_query("SELECT first_name || age FROM people WHERE id = 1").unwrap();
    assert!(
...
fn test_concat_in_where_clause() {
    let _g = setup();
    let result =
        process_query("SELECT id FROM people WHERE first_name || ' ' || last_name = 'John Doe'")
            .unwrap();
...
fn test_concat_with_alias() {
    ...
    let result = process_query(
        "SELECT first_name || ' ' || last_name AS full_name FROM people WHERE id = 2",
    )
    .unwrap();
    assert!(
...
fn test_concat_function() {
    let _g = setup();
    let result =
        process_query("SELECT CONCAT(first_name, ' ', last_name) FROM people WHERE id = 1")
            .unwrap();
    assert!(
        result.contains("John Doe"),
        "Expected 'John Doe', got: {}",
