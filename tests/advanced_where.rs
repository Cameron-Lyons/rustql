...
fn test_in_operator() {
    ...
    let result =
        process_query("SELECT name FROM users WHERE name IN ('Alice', 'Charlie')").unwrap();
    assert!(result.contains("Alice"));
    assert!(result.contains("Charlie"));
    assert!(!result.contains("Bob"));
}
...
fn test_like_operator() {
    ...
    let result =
        process_query("SELECT name FROM products WHERE category LIKE '%Electron%'").unwrap();
    assert!(result.contains("Laptop Computer"));
    assert!(result.contains("Desktop PC"));
    assert!(!result.contains("Mouse Pad"));
...
fn test_between_operator() {
    ...
    let result =
        process_query("SELECT name FROM employees WHERE salary BETWEEN 50000 AND 75000").unwrap();
    assert!(result.contains("Alice"));
    assert!(result.contains("Bob"));
    assert!(!result.contains("Charlie"));
    assert!(!result.contains("David"));
    ...
    let result =
        process_query("SELECT name FROM employees WHERE salary BETWEEN 100000 AND 125000").unwrap();
    assert!(!result.contains("Alice"));
    assert!(!result.contains("Bob"));
    assert!(result.contains("Charlie"));
    assert!(result.contains("David"));
...
fn test_between_with_text() {
    ...
    let result =
        process_query("SELECT name FROM items WHERE name BETWEEN 'Apple' AND 'Banana'").unwrap();
    assert!(result.contains("Apple"));
    assert!(result.contains("Banana"));
...
fn test_combined_operators() {
    ...
    process_query("CREATE TABLE students (id INTEGER, name TEXT, age INTEGER, grade TEXT)")
        .unwrap();
...
fn test_not_in() {
    ...
    let result =
        process_query("SELECT name FROM users WHERE NOT (role IN ('Guest', 'User'))").unwrap();
    assert!(result.contains("Alice"));
    assert!(!result.contains("Bob"));
    assert!(!result.contains("Charlie"));
}
...
fn test_like_not_like() {
    ...
    process_query("INSERT INTO products VALUES (1, 'laptop'), (2, 'desktop'), (3, 'mouse')")
        .unwrap();
